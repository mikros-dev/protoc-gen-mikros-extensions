...
// Wire represents the 'api/wire.tmpl' importer
type Wire struct{}

// Name returns the template name.
func (w *Wire) Name() spec.Name {
	return spec.NewName("api", "wire")
}

// Load returns a slice of imports for the template.
func (w *Wire) Load(ctx *Context, cfg *settings.Settings) []*Import {
	...
	for k, v := range w.loadImportsFromMessagesToWire(ctx, cfg, ctx.DomainMessages) {
		imports[k] = v
	}
...
func (w *Wire) loadImportsFromMessagesToWire(
	ctx *Context,
	...
	for _, msg := range messages {
		for _, f := range msg.Fields {
			w.processField(ctx, cfg, msg.Receiver, f, imports)
		}
	}
